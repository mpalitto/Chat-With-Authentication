function authController($scope, $http, $window, $templateCache,  UserService, ChatService) {
    $scope.authData = {};
        ...
        $http.post('/'+type, $scope.authData)
            .success(function(messages) {
               ...
               if( $scope.authData.username == 'admin') {
                   ...
                   $templateCache.remove('/partials/usersprofile.ejs');
                   $window.location.href = "#/users";
               } else {
                   console.log('redirecting to chat panel');
                   $templateCache.remove('/partials/chatprofile.ejs');
