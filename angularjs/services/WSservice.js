mainApp.factory('ChatService', function() {
  ...
  service.connect = function(username) {
    ...
    ws.onopen = function() {
      ws.send(username);
      service.callback({id: 'connection', from: 'sys', msg: 'connected', type: 'alert alert-success'});
    };
 
    ws.onclose = function() {
      service.callback({id: 'connection', from: 'sys', msg: 'connection closed', type: 'alert alert-danger'});
    };
 
    ws.onerror = function() {
      service.callback({id: 'connection', from: 'sys', msg: 'Failed to open a connection', type: 'alert alert-danger'});
    }
